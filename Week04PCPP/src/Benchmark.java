...
class Benchmark {
	...
	public static void main(String[] args) {
		...
		double i = Mark7("Memoizer", new IntToDouble() {
			@Override
			public double call(int i) {
				Factorizer f = new Factorizer();
				Exercise_3_2.exerciseFactorizer(new Memoizer<Long, long[]>(f));
				return f.getCount();
			}
		});
	...
	private static void testMemoizer1_2_3_4() {
		double i = Mark7("Memoizer1", new IntToDouble() {
			@Override
			public double call(int i) {
				Factorizer f = new Factorizer();
				return Exercise_3_2
		...
		System.out.println(i);
		i = Mark7("Memoizer2", new IntToDouble() {
			@Override
			public double call(int i) {
				Factorizer f = new Factorizer();
				return Exercise_3_2
		...
		System.out.println(i);
		i = Mark7("Memoizer3", new IntToDouble() {
			@Override
			public double call(int i) {
				Factorizer f = new Factorizer();
				return Exercise_3_2
		...
		System.out.println(i);
		i = Mark7("Memoizer4", new IntToDouble() {
			@Override
			public double call(int i) {
				Factorizer f = new Factorizer();
				return Exercise_3_2
	...
	private static void MathFunctionBenchmarks() {
		Mark7("pow", new IntToDouble() {
			public double call(int i) {
				return Math.pow(10.0, 0.1 * (i & 0xFF));
			}
		});
		Mark7("exp", new IntToDouble() {
			public double call(int i) {
				return Math.exp(0.1 * (i & 0xFF));
			}
		});
		Mark7("log", new IntToDouble() {
			public double call(int i) {
				return Math.log(0.1 + 0.1 * (i & 0xFF));
			}
		});
		Mark7("sin", new IntToDouble() {
			public double call(int i) {
				return Math.sin(0.1 * (i & 0xFF));
			}
		});
		Mark7("cos", new IntToDouble() {
			public double call(int i) {
				return Math.cos(0.1 * (i & 0xFF));
			}
		});
		Mark7("tan", new IntToDouble() {
			public double call(int i) {
				return Math.tan(0.1 * (i & 0xFF));
			}
		});
		Mark7("asin", new IntToDouble() {
			public double call(int i) {
				return Math.asin(1.0 / 256.0 * (i & 0xFF));
			}
		});
		Mark7("acos", new IntToDouble() {
			public double call(int i) {
				return Math.acos(1.0 / 256.0 * (i & 0xFF));
			}
		});
		Mark7("atan", new IntToDouble() {
			public double call(int i) {
				return Math.atan(1.0 / 256.0 * (i & 0xFF));
			}
		});
	}

	private static void SearchBenchmarks() {
		final int[] intArray = SearchAndSort.fillIntArray(10_000); // sorted
																	// [0,1,...]
		final int successItem = 4900, failureItem = 14000;
		Mark7("linear_search_success", new IntToDouble() {
			public double call(int i) {
				return SearchAndSort.linearSearch(successItem, intArray);
			}
		});
		Mark7("binary_search_success", new IntToDouble() {
			public double call(int i) {
	...
	private static void SearchScalabilityBenchmarks1() {
		for (int size = 100; size <= 10_000_000; size *= 2) {
			final int[] intArray = SearchAndSort.fillIntArray(size); // sorted
																		// [0,1,...]
			final int successItem = (int) (0.49 * size);
			Mark8("binary_search_success", String.format("%8d", size),
					new IntToDouble() {
						public double call(int i) {
							return SearchAndSort.binarySearch(successItem,
									intArray);
						}
					});
		}
	...
	private static void GetPseudorandomItems() {
		for (int size = 100; size <= 10_000_000; size *= 2) {
			...
			final int n = size;
			SearchAndSort.shuffle(items);
			Mark8("get_pseudorandom_items", String.format("%8d", size),
					new IntToDouble() {
						public double call(int i) {
							int successItem = items[i % n];
							return successItem;
	...
	private static void SortingBenchmarks() {
		...
		Mark7("shuffle int", new IntToDouble() {
			public double call(int i) {
	...
	private static void SortingScalabilityBenchmarks() {
		for (int size = 100; size <= 50000; size *= 2) {
			...
			Mark8Setup("selection_sort", String.format("%8d", size),
					new Benchmarkable() {
						public void setup() {
							SearchAndSort.shuffle(intArray);
						}
		...
		for (int size = 100; size <= 2000000; size *= 2) {
			final int[] intArray = SearchAndSort.fillIntArray(size);
			Mark8Setup("quicksort", String.format("%8d", size),
					new Benchmarkable() {
						public void setup() {
							SearchAndSort.shuffle(intArray);
						}
		...
		for (int size = 100; size <= 2000000; size *= 2) {
			final int[] intArray = SearchAndSort.fillIntArray(size);
			Mark8Setup("heapsort", String.format("%8d", size),
					new Benchmarkable() {
						public void setup() {
							SearchAndSort.shuffle(intArray);
						}
	...
	public static double Mark8(String msg, String info, IntToDouble f, int n,
			double minTime) {
		...
		double mean = st / n, sdev = Math.sqrt(sst / n - mean * mean);
		System.out.printf("%-25s %s%15.1f ns %10.2f %10d%n", msg, info, mean,
				sdev, count);
		return dummy / totalCount;
	}
